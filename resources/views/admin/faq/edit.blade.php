<form method="post" class="bg-warning p-2 border-lite" action="{{ route('manage-faq.update', $data->id) }}"
    ...
    <div class="col-12 mb-3">
        <div class="form-group">
            ...
            <textarea class="form-control custom-input @error('answer') is-invalid @enderror" id="answer" autocomplete="off"
                name="answer" placeholder="Answer" rows="8">@purify($data->answer)</textarea>
            @error('answer')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
